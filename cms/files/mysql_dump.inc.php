...
class MYSQL_DUMP
{
	...
	function dumpDB($database = HAR_ALL_DB, $tables = HAR_ALL_TABLES, $options = HAR_ALL_OPTIONS)
	{
		...
		$returnSql .= "USE `$this->database`;" . $lineEnd . $lineEnd;
		
		$this->result = @DB::Query( "USE `$this->database`");
	...
	function _connect()
	{
		if (! is_resource ( $this->conn ))
			$this->conn = @DB::Connect( $this->dbhost, $this->dbuser, $this->dbpwd );
		if (! is_resource ( $this->conn ))
		{
