...
class xajaxArgumentManager
{
	...
	function argumentDecodeXML(&$sArg)
	{
		...
		while ( $nCurrent < $nLength )
		{
					...
					if ($sExpecting == substr ( $sArg, $nCurrent, $nExpectedLength ))
					{
						...
						switch ($nFound)
						{
							case 3 :
								...
								break;
							case 4 :
								$sKey = str_replace ( array ('<' . '![CDATA[', ']]>' ), '', substr ( $sArg, $nLast, $nCurrent - $nLast ) );
								break;
							case 5 :
								$mValue = '';
								break;
							case 6 :
								if ($nLast < $nCurrent)
								{
									$mValue = str_replace ( array ('<' . '![CDATA[', ']]>' ), '', substr ( $sArg, $nLast, $nCurrent - $nLast ) );
									$cType = substr ( $mValue, 0, 1 );
									$sValue = substr ( $mValue, 1 );
									switch ($cType)
									{
										case 'S' :
											$mValue = false === $sValue ? '' : $sValue;
											break;
										case 'B' :
											$mValue = $this->convertStringToBool ( $sValue );
											break;
										case 'N' :
											$mValue = floatval ( $sValue );
											break;
										case '*' :
											$mValue = null;
											break;
			...
			if (false == $bFound)
			{
				if (0 == $nCurrent)
				{
					$sArg = str_replace ( array ('<' . '![CDATA[', ']]>' ), '', $sArg );
					$cType = substr ( $sArg, 0, 1 );
					$sValue = substr ( $sArg, 1 );
					switch ($cType)
					{
						case 'S' :
							$sArg = false === $sValue ? '' : $sValue;
							break;
						case 'B' :
							$sArg = $this->convertStringToBool ( $sValue );
							break;
						case 'N' :
							$sArg = floatval ( $sValue );
							break;
						case '*' :
							$sArg = null;
							break;
		...
		$objLanguageManager = &xajaxLanguageManager::getInstance ();
		trigger_error ( $objLanguageManager->getText ( 'ARGMGR:ERR:01' ) . $sExpecting . $objLanguageManager->getText ( 'ARGMGR:ERR:02' ) . $sArg, E_USER_ERROR );
	}
	...
	function xajaxArgumentManager()
	{
		...
		$this->aSequence = array ('<' . 'k' . '>' => array (array ('<' . '/k' . '>' => 4 ), 3 ), '<' . '/k' . '>' => array (array ('<' . 'v' . '>' => 3, '<' . '/e' . '>' => 4 ), 4 ), '<' . 'v' . '>' => array (array ('<' . 'xjxobj' . '>' => 8, '<' . '/v' . '>' => 4 ), 5 ), '<' . '/v' . '>' => array (array ('<' . '/e' . '>' => 4, '<' . 'k' . '>' => 3 ), 6 ), '<' . 'e' . '>' => array (array ('<' . 'k' . '>' => 3, '<' . 'v' . '>' => 3, '<' . '/e' . '>' => 4 ), 2 ), '<' . '/e' . '>' => array (array ('<' . 'e' . '>' => 3, '<' . '/xjxobj' . '>' => 9 ), 7 ), '<' . 'xjxobj' . '>' => array (array ('<' . 'e' . '>' => 3, '<' . '/xjxobj' . '>' => 9 ), 1 ), '<' . '/xjxobj' . '>' => array (array ('<' . '/v' . '>' => 4 ), 8 ), 'start' => array (array ('<' . 'xjxobj' . '>' => 8 ), 9 ) );
...
class xajax
{
	...
	function xajax($sRequestURI = null, $sLanguage = null)
	{
		...
		$this->sLogFile = '';
		$this->__wakeup ();
		$this->configureMany ( array ('characterEncoding' => XAJAX_DEFAULT_CHAR_ENCODING, 'decodeUTF8Input' => false, 'outputEntities' => false, 'defaultMode' => 'asynchronous', 'defaultMethod' => 'POST', 'wrapperPrefix' => 'xajax_', 'debug' => false, 'verbose' => false, 'useUncompressedScripts' => false, 'statusMessages' => false, 'waitCursor' => true, 'scriptDeferral' => false, 'exitAllowed' => true, 'errorHandler' => false, 'cleanBuffer' => false, 'allowBlankResponse' => false, 'allowAllResponseTypes' => false, 'generateStubs' => true, 'logFile' => '', 'timeout' => 6000, 'version' => $this->getVersion () ) );
		if (null !== $sRequestURI)
	...
	function _detectURI()
	{
		...
		$querySbyVazgen = (isset($aURL ['query'])) ? $aURL ['query'] : '';
		$sURL .= $aURL ['path'] . $querySbyVazgen;
		unset ( $aURL );
		$aURL = explode ( "?", $sURL );
		if (1 < count ( $aURL ))
		{
...
class xajaxResponse
{
	...
	{
		return $this->addCommand ( array ('cmd' => 'cc', 'id' => $iCmdNumber ), $sMessage );
	}
	function assign($sTarget, $sAttribute, $sData)
	{
		return $this->addCommand ( array ('cmd' => 'as', 'id' => $sTarget, 'prop' => $sAttribute ), $sData );
	}
	function append($sTarget, $sAttribute, $sData)
	{
		return $this->addCommand ( array ('cmd' => 'ap', 'id' => $sTarget, 'prop' => $sAttribute ), $sData );
	}
	function prepend($sTarget, $sAttribute, $sData)
	{
		return $this->addCommand ( array ('cmd' => 'pp', 'id' => $sTarget, 'prop' => $sAttribute ), $sData );
	}
	function replace($sTarget, $sAttribute, $sSearch, $sData)
	{
		return $this->addCommand ( array ('cmd' => 'rp', 'id' => $sTarget, 'prop' => $sAttribute ), array ('s' => $sSearch, 'r' => $sData ) );
	}
	...
	{
		return $this->addCommand ( array ('cmd' => 'c:as', 'prop' => $sAttribute ), $sData );
	}
	function contextAppend($sAttribute, $sData)
	{
		return $this->addCommand ( array ('cmd' => 'c:ap', 'prop' => $sAttribute ), $sData );
	}
	function contextPrepend($sAttribute, $sData)
	{
		return $this->addCommand ( array ('cmd' => 'c:pp', 'prop' => $sAttribute ), $sData );
	}
	...
	function alert($sMsg)
	{
		return $this->addCommand ( array ('cmd' => 'al' ), $sMsg );
	}
	function debug($sMessage)
	{
		return $this->addCommand ( array ('cmd' => 'dbg' ), $sMessage );
	}
	...
	function script($sJS)
	{
		return $this->addCommand ( array ('cmd' => 'js' ), $sJS );
	}
	function call()
	{
		$aArgs = func_get_args ();
		$sFunc = array_shift ( $aArgs );
		return $this->addCommand ( array ('cmd' => 'jc', 'func' => $sFunc ), $aArgs );
	}
	function remove($sTarget)
	{
		return $this->addCommand ( array ('cmd' => 'rm', 'id' => $sTarget ), '' );
	}
	function create($sParent, $sTag, $sId, $sType = null)
	{
		...
		return $this->addCommand ( array ('cmd' => 'ce', 'id' => $sParent, 'prop' => $sId ), $sTag );
	}
	function insert($sBefore, $sTag, $sId)
	{
		return $this->addCommand ( array ('cmd' => 'ie', 'id' => $sBefore, 'prop' => $sId ), $sTag );
	}
	function insertAfter($sAfter, $sTag, $sId)
	{
		return $this->addCommand ( array ('cmd' => 'ia', 'id' => $sAfter, 'prop' => $sId ), $sTag );
	}
	function createInput($sParent, $sType, $sName, $sId)
	{
		return $this->addCommand ( array ('cmd' => 'ci', 'id' => $sParent, 'prop' => $sId, 'type' => $sType ), $sName );
	}
	function insertInput($sBefore, $sType, $sName, $sId)
	{
		return $this->addCommand ( array ('cmd' => 'ii', 'id' => $sBefore, 'prop' => $sId, 'type' => $sType ), $sName );
	}
	function insertInputAfter($sAfter, $sType, $sName, $sId)
	{
		return $this->addCommand ( array ('cmd' => 'iia', 'id' => $sAfter, 'prop' => $sId, 'type' => $sType ), $sName );
	}
	function setEvent($sTarget, $sEvent, $sScript)
	{
		return $this->addCommand ( array ('cmd' => 'ev', 'id' => $sTarget, 'prop' => $sEvent ), $sScript );
	}
	...
	{
		return $this->addCommand ( array ('cmd' => 'ah', 'id' => $sTarget, 'prop' => $sEvent ), $sHandler );
	}
	function removeHandler($sTarget, $sEvent, $sHandler)
	{
		return $this->addCommand ( array ('cmd' => 'rh', 'id' => $sTarget, 'prop' => $sEvent ), $sHandler );
	}
	function setFunction($sFunction, $sArgs, $sScript)
	{
		return $this->addCommand ( array ('cmd' => 'sf', 'func' => $sFunction, 'prop' => $sArgs ), $sScript );
	}
	function wrapFunction($sFunction, $sArgs, $aScripts, $sReturnValueVariable)
	{
		return $this->addCommand ( array ('cmd' => 'wpf', 'func' => $sFunction, 'prop' => $sArgs, 'type' => $sReturnValueVariable ), $aScripts );
	}
	...
	{
		$this->addCommand ( array ('cmd' => 'rjs', 'unld' => $sUnload ), $sFileName );
		return $this;
	}
	...
	{
		$sData = "";
		$this->addCommand ( array ('cmd' => 'wcss', 'prop' => $iTimeout ), $sData );
		return $this;
	}
	function waitFor($script, $tenths)
	{
		return $this->addCommand ( array ('cmd' => 'wf', 'prop' => $tenths ), $script );
	}
	function sleep($tenths)
	{
		$this->addCommand ( array ('cmd' => 's', 'prop' => $tenths ), '' );
		return $this;
	}
	...
	function _encodeArray(&$mData)
	{
		...
		foreach ( array_keys ( $mData ) as $sKey )
			$aData [] = array ('k' => $sKey, 'v' => $this->_encodeArray ( $mData [$sKey] ) );
		return $aData;
	}
}
	...
	function printJavascriptInclude()
	{
		...
		if (0 < $this->nScriptLoadTimeout)
		{
			foreach ( $aJsFiles as $aJsFile )
			{
				...
				echo ' function() {';
				echo $sCrLf;
				echo '  public scriptExists = false;';
				echo $sCrLf;
				echo '  try { if (';
				echo $aJsFile [1];
